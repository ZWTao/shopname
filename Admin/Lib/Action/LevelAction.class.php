	...
	class LevelAction extends Action{
		...
		function sendbanner(){
			...
			if(!in_array('35',$_SESSION['level'])){
				...
				$data['userid'] = $_SESSION['id'];
				$data['time'] = time();
				$data['loginfo'] = '管理员'.$_SESSION['username'].'发布了banner配图';
				$data['userip'] = $_SERVER['REMOTE_ADDR'];
				$log->add($data);
			}
		}
		...
		function sendspecial(){
			...
			if(!in_array('36',$_SESSION['level'])){
				...
				$data['userid'] = $_SESSION['id'];
				$data['time'] = time();
				$data['loginfo'] = '管理员'.$_SESSION['username'].'发布了特价旅游';
				$data['userip'] = $_SERVER['REMOTE_ADDR'];
				$log->add($data);
			}
		}
		...
		function sendproject(){
			...
			if(!in_array('35',$_SESSION['level'])){
				...
				$data['userid'] = $_SESSION['id'];
				$data['time'] = time();
				$data['loginfo'] = '管理员'.$_SESSION['username'].'发布了旅游项目';
				$data['userip'] = $_SERVER['REMOTE_ADDR'];
				$log->add($data);
			}
		}
		...
		function sendactivity(){
			...
			if(!in_array('35',$_SESSION['level'])){
				...
				$data['userid'] = $_SESSION['id'];
				$data['time'] = time();
				$data['loginfo'] = '管理员'.$_SESSION['username'].'发布了热门活动';
				$data['userip'] = $_SERVER['REMOTE_ADDR'];
				$log->add($data);
			}
		}
